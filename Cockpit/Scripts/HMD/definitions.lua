...
function AddCircle(xpos, ypos, radius, border, parent_element, color)
	...
	for i=1, border do
			...
			RWR_circle_i.level			= HMD_DEFAULT_LEVEL
...
function Add_HMD_VIDEO_Element(object)
	object.use_mipfilter      = true
	object.h_clip_relation    = h_clip_relations.COMPARE
	object.level			  = HMD_DEFAULT_LEVEL
...
function create_bg_textured_hud(ind_material,UL_X,UL_Y,DR_X,DR_Y,CENTER_X,CENTER_Y, scale, tex_size)
...
function Rectangle(width, parent_element, h, w)
	...
	rightBoxLine.element_params  = {"HMD_STATE" }
